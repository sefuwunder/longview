...
//   parallel — Parallel's official JSON API (needs PARALLEL_API_KEY;
//         see src/backends/parallel.ts)
//
...
import { searchParallel, parallelKeyConfigured } from "./backends/parallel";

export type BackendName = "ddg" | "exa" | "parallel";

/** DDG crawl classes plus the API-specific classes the Exa/Parallel backends add. */
...
export interface SearchOutcome {
  ...
  backend: BackendName;
  /** DDG endpoint name ("html-post" | "html-get" | "lite" | "custom"), "exa-api", or "parallel-api". */
...
export function resolveBackend(db: Database | null): BackendSelection {
  ...
  if (env === "ddg" || env === "exa" || env === "parallel")
  ...
  if (db) {
    ...
    if (s === "ddg" || s === "exa" || s === "parallel")
      return { name: s, source: "setting" };
  }
...
export async function search(
  ...
  if (name === "parallel") return searchParallel(query, opts);
...
export interface SearchDiag {
  ...
  /** Exa/Parallel only: whether the API key is set. Null for DDG. */
  keyConfigured: boolean | null;
}

/**
 * Probe the active backend once per endpoint and report status. For Exa and
 * Parallel this costs one search call; for DDG it walks the existing
 * fallback chain.
 */
export async function diagnoseSearch(
  ...
  if (name === "parallel") {
    const r = await searchParallel(query, { maxResults: 3 });
    return {
      backend: "parallel",
      query,
      winner: r.ok ? "parallel-api" : null,
      endpoints: [
        {
      ...
      keyConfigured: parallelKeyConfigured(),
    };
  }
